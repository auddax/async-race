...
class Winners extends Loader implements IWinners {
  button: IButton;

  constructor() {
    super(environment.baseUrl, Path.WINNERS);
  ...
  async createWinner(winnerCurrent: ICar): Promise<Response> {
    const responseWinner = await this.getWinner(Number(winnerCurrent.id));

    if (responseWinner.status === 200) {
      const winnerSaved = await responseWinner.json();
      const winnerNew: ICar = {
        id: Number(winnerCurrent.id),
        wins: winnerSaved.wins + 1,
        time: winnerCurrent.time < winnerSaved.time ? winnerCurrent.time : winnerSaved.time,
        name: winnerCurrent.name,
        color: winnerCurrent.color,
      };
      const response = await this.updateWinner(winnerNew);
      return response;
    }
  ...
  async updateWinner(winnerCar: ICar): Promise<Response> {
    const requestHeaders = new Headers();
    requestHeaders.append('Content-Type', 'application/json');
    const {
      wins, time, name, color,
    } = winnerCar;
    const requestOptions = {
      method: 'PUT',
      headers: requestHeaders,
    ...
    const response = await super.getResponse(Number(winnerCar.id), {}, requestOptions);
    return response;
  }

  async deleteWinner(id: number): Promise<Response> {
    const requestOptions = {
      method: 'DELETE',
    };
  ...
  async render(page?: number) {
    ...
    return (`
      ...
      <thead class="table__header">
        <tr>
          <th id="winnersNumber">№</th>
          <th id="winnersCar">Car</th>
          <th id="winnersName">Name</th>
          <th id="winnersNumber">Wins</th>
          <th id="winnersBestTime">Best time (sec)</th>
        </tr>
      </thead>
      <tbody>
        ${content}
