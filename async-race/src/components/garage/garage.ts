...
class Garage extends Loader implements IGarage {
  ...
  async getCars(page = 1, limit = 7) {
    const response = await super.loader(undefined, { _page: page, _limit: limit });
    return {
      items: response.json(),
  ...
  async render(page?: number) {
    return (await (await this.getCars(page)).items).map((item: ICar) => `
      <div id=${item.id}>
        <input type="radio" id="selectCar${item.id}" name="car">
        <button type="button" class="button">
          <label for="selectCar${item.id}">Select</label>
        </button>
        ${this.button.render('Remove', 'deleteCarButton')}
        <h4>Model: ${item.name}</h4>
        ${this.buttonEngine.render('A', `engineA${item.id}`)}
        ${this.buttonEngine.render('B', `engineB${item.id}`)}
        <svg width="176" height="50" viewBox="0 0 176 50" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
